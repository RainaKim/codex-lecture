...
- 자연스러운 한국어로 쓴다. 번역투와 명사 쌓기(「요청 구조 고정 및 검증 수행」 같은 것)는 쓰지 않는다
- **슬라이드 문구는 개조식으로 쓴다.** `~다`로 끝내지 않고 `드러남`·`잡기 어려움`·`판정 가능`처럼
  맺는다. 제목도 마찬가지다 — 「그럴듯할수록 위험하다」가 아니라 「그럴듯함의 위험」.
  **스피커 노트는 예외다** — 소리 내어 읽는 대본이므로 `~니다` 구어체를 쓴다
