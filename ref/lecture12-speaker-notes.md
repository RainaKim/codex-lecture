...
**3) `[두 줄 사이를 손가락으로 왔다 갔다 한다. 여기가 이 장의 전부다]`**
> "그러면 **이 줄에서 이 줄까지 시간이 얼마나 흐를까요?**"
> `[한 박자 기다린다]`
> "**1초일 수도 있고 10분일 수도 있습니다.** 장바구니에 담아 두고 **커피 마시러 갔다 올 수도**
> 있잖아요. **코드에는 이 사이에 아무것도 없습니다.** 그냥 **비어 있어요.**"

**4) `[왼쪽 src/orders.ts:59~64로 돌아온다]`**
> "그런데 지금 코드는 **주문을 만들 때 재고를 깎아 둡니다.** 그러니까 **자리를 미리 잡아 두는
> 셈**이죠. 오늘 우리가 하려는 건 **이걸 없애는 겁니다.** 결제에 성공해야 깎는 걸로요."
> "그럼 **깎지 않고 기다리는 동안** 어떻게 되죠? **다른 사람이 같은 상품을 주문하면 그 재고를
> 가져갈 수 있습니다.**"

**5) `[다시 두 줄을 번갈아 짚으며 결론]`**
> "그래서 **검사가 두 번** 필요합니다. **주문을 만들 때 한 번** — 지금 이 주문을 받아도 되는지.
> **결제 직전에 한 번 더** — 지금 결제를 성공시켜도 되는지."
> "**처음 검사를 통과했다고 두 번째도 통과하는 게 아닙니다.** 그 사이에 시간이 흘렀으니까요."
