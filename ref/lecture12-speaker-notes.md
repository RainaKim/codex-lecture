...
> "Issue는 **무엇을 바꿀지** 정하고, 정책은 **어떤 동작이 허용되는지** 정합니다. 계획은 **어디를 어떤
> 순서로 바꾸고 무엇으로 확인할지** 연결합니다. 구현이 끝나면 **실제 diff가 그 계획과 맞는지** 다시
> 보는 흐름입니다."

⚠️ **흐름도에서 두 곳을 반드시 짚는다.**
> "먼저 가운데의 **승인 지점**입니다. 계획을 잘 썼다고 바로 코드를 바꾸는 게 아니에요. 사람이 필요한
> 결정과 범위를 확인한 뒤에 구현으로 내려갑니다."
> "그리고 구현에서 **화살표가 다시 돌아옵니다.** 계획대로만 흘러갈 거라고 가정하지 않는 겁니다.
> 실제 변경을 보고 예상과 다르면 **멈추고 판단한 뒤** 다시 진행합니다."

> "E1부터 E4까지 순서가 있다고 **모든 단계를 무조건 끝까지 밀어붙이는 건 아닙니다.** E3에서 범위가
> 커졌다면 **E4의 판단 기준을 먼저** 적용합니다. 아직 정해지지 않은 동작이 필요하면 거기서 **사람에게
> 돌아옵니다.**"
...
> "앱 코드가 그대로인지는 **일반 diff뿐 아니라 staged 변경과 새 파일까지** 같이 확인하겠습니다."
...
> "Issue의 문장을 계획 파일에 **그대로 옮기면** 계획이 됐을까요? 무엇을 해야 하는지는 남겠지만,
> **왜 이 파일을 바꾸는지**와 **끝났다는 걸 무엇으로 확인할지**는 빠질 수 있습니다."

> "9강에서 만든 template은 **앞으로 어떤 작업에도 쓸 질문 목록**이었습니다. 지금은 그 질문에 **이번
> Issue의 답**을 넣는 겁니다. 양식을 다시 만드는 게 아니라 **실제 계획을 작성하는 첫 단계**죠."

## 2. 강사용 표 — 연결 고리 (README에서 뺀 자료)

`[화면: 아래 표]` 한 완료 기준과 한 변경 후보를 골라 연결만 보여 준다.
...
⚠️ **표를 위에서 아래로만 읽지 않는다.**
> "완료 기준에서 출발해 정책과 현재 코드, 변경 책임, 관찰 결과로 **내려갈 수 있어야** 하고,
> 변경 후보에서 출발해 **왜 필요한지 거꾸로 찾을 수도** 있어야 합니다."
> "이걸 **요구사항의 추적 가능성**이라고 합니다. 말은 딱딱한데 뜻은 간단해요 — **'이 변경은 왜
> 들어갔죠?'라고 물었을 때 원래 요청과 근거로 돌아갈 수 있다**는 겁니다."

> "Issue와 정책의 **역할도 다릅니다.** Issue는 **이번에 해결할 일**을 정하고, 정책은 **그 일을 할 때
> 지킬 조건**을 정합니다. 장기 정책에 환불 상태가 있다고 **이번 작업에 환불 기능까지 넣는 건 아닙니다.**"

> "현재 코드의 차이는 그 사이를 연결합니다. 목표와 지금 동작이 어떻게 다른지 알아야 바꿀 책임을 정할
> 수 있어요. **파일 목록은 그 조사 결과여야지, 파일 이름이 눈에 띄었다고 정하는 목록이 아닙니다.**"

> "마지막 칸은 **관찰 결과**입니다. '개선한다'로 끝내지 않고, **응답이나 저장 상태, 재고 수량, 이력**
> 중 무엇이 달라지는지 적습니다. 이 결과를 E2에서 검증 방법과 연결하겠습니다."

**강사 진행** — 다섯 행을 모두 읽지 않는다. **실제 제안의 한 항목**에서 완료 기준·정책·변경 책임·
관찰 결과를 찾아 연결한다. **별도 관계표 파일을 만들지 않는다.**
...
`[화면: Goal의 생성·성공·실패 결과 → 미정 표시 → 읽을 파일과 변경 후보 비교]`
...
## 3. 강사용 도식 — 재고 검사 시점 (README에서 뺀 자료)
...
## 4. 강사용 표 — 검증 수단과 한계 (README에서 뺀 자료)

각 행을 모두 읽지 않고 **실제 리뷰 결과와 관련된 두 행만** 대조한다.
