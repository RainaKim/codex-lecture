...
⚠️ **여유가 있으면 한 겹 더 본다 — 성공 경로만 적혀 있지 않은가.**
잘 빠지는 것: **부분 실패**(여러 줄 중 일부만 처리되는 경우), **경계**(딱 맞게 떨어지는 수량),
**중복 입력**(같은 대상이 여러 줄로 들어오는 경우). 하나라도 비어 있으면 그 자리에서 말한다.
> "성공했을 때는 적혀 있는데, **중간에 실패하면 어디까지 되돌아가는지**가 없네요.
> 이런 건 **구현하다 만나면 늦습니다.** 지금 적어 두죠."
...
## 6. 이제 저장한다 — 저장은 요청을 한 번 더 보내는 일이다

⚠️ **여기서 헷갈리기 쉽다. Plan mode를 끄는 것만으로는 파일이 안 생긴다.**
지금까지 본 건 **채팅에 뜬 제안**이고, **계획 파일은 아직 없다.** 저장하려면 **요청을 한 번 더**
보내야 한다. **이 사실 자체가 오늘 가르칠 내용**이다 — Plan mode가 파일을 못 만드는 모드라는 게
E2의 승인 경계로 그대로 이어진다.

**1) `[Composer 하단의 Plan mode 표시를 짚으며]`**
> "읽어 봤고 쓸 만하니 저장하겠습니다. **여기 Plan mode를 먼저 끕니다.**"

⚠️ **끄자마자 한마디 붙인다. 안 붙이면 학생이 '수락 = 저장'으로 오해한다.**
> "그런데 **끈다고 파일이 생기는 건 아닙니다.** 지금 화면에 있는 건 **채팅에 뜬 제안**이에요.
> **Plan mode는 원래 파일을 못 만드는 모드**였잖아요. 그래서 **저장해 달라고 한 번 더 요청**합니다.
> **읽는 단계와 쓰는 단계가 이렇게 갈라져 있는 것**, 이게 오늘 계속 나올 구조입니다."

**2) `[일반 모드에서 아래 요청을 보낸다]`**

```text
방금 Plan mode에서 검토한 내용을 @docs/templates/implementation-plan.md 의 구조 그대로
notes/plans/l12-inventory-after-payment.md 에 저장해라.
여덟 영역의 제목을 모두 그대로 두고, 채우는 것은 Goal, Files to Read First, Files to Change
세 영역뿐이다.
나머지 다섯 영역(Steps, Tests to Add or Update, Risks, Out of Scope, Stop Conditions)은
template의 안내 문구를 그대로 남겨라. 내용을 지어내지 마라.
미정으로 둔 항목은 미정 표시와 결정할 사람을 함께 적어라.
이 파일 하나만 만들고 src/ 와 tests/ 는 어떤 파일도 수정하지 마라.
```

⚠️ **요청을 보내기 전에 세 군데를 짚으며 왜 이렇게 쓰는지 말한다.**

| 짚을 줄 | 할 말 |
| --- | --- |
| `여덟 영역의 제목을 모두 그대로` | "**틀은 통째로** 가져갑니다. 오늘 안 채우는 칸도 **자리는 남겨 둬야** 나중에 뭘 안 했는지 보이니까요." |
| `나머지 다섯 영역은 … 지어내지 마라` | "**이게 제일 중요합니다.** 안 시키면 **친절하게 다 채워 줍니다.** 근데 그건 **승인 안 된 내용**이에요." |
| `src 와 tests 는 … 수정하지 마라` | "**계획을 저장하는 요청에 코드 수정이 딸려 오면 안 됩니다.** 말로 못 박아 둡니다." |

⚠️ **5장 검토에서 두 갈래로 읽히는 표현을 찾았다면, 그때만 한 줄 덧붙인다.**
```text
그리고 「(그 표현)」이 두 가지로 읽히니, 어느 쪽인지 한 줄로 분명히 적어라.
```
> "아까 **애매하다고 짚은 문장**, **저장하면서 같이 정리**합니다. **읽고 나서 고칠 수 있는 게
> 저장 전에 검토한 이유**예요."

**3) `[README E1 `실행`으로 이동 — 요청 블록 아래 bullet 두 줄을 차례로 짚는다]`**
> "방금 요청이 README와 맞는지 보겠습니다. **`notes/plans/l12-inventory-after-payment.md`에
> 저장**, **나머지 다섯 영역은 template 안내만 유지** — 그대로죠."
> `[바로 아래 줄로 커서를 내리며]`
