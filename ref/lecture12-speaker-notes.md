...
## 6. 이제 저장한다 — 저장과 승인을 가른다

⚠️ **오늘 여러 번 반복할 구분이다. 세 군데를 차례로 짚는다.**

**1) `[Composer 하단의 Plan mode 표시를 짚으며]`**
> "읽어 봤고 쓸 만하니, **이제 여기 Plan mode를 종료**하고 저장하겠습니다."

**2) `[README E1 `실행`으로 이동 — 요청 블록 아래 bullet 두 줄을 차례로 짚는다]`**
> "어디에 저장할지는 README에 있습니다. **`notes/plans/l12-inventory-after-payment.md`**,
> 그리고 **나머지 다섯 영역은 template 안내를 그대로 둡니다.**"
> `[바로 아래 줄로 커서를 내리며]`
> "그다음 줄이 오늘 계속 나올 문장이에요. **'계획 저장만 허용하며 `src`·`tests`는 변경하지 않는다.
> 구현 승인은 E2에서 별도로 진행한다.'**"

**3) `[파일 트리에서 방금 생긴 계획 파일 하나만 가리키며]`**
> "지금 늘어난 건 **이 파일 하나**입니다. **앱 코드는 한 줄도 안 바뀌었어요.**
