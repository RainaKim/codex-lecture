...
### README 수행 순서와 이 노트의 대응 (강사 조회용)

⚠️ **README E3에는 「수행 순서」 네 줄이 있다.** 노트가 그 네 줄을 어디서 처리하는지 적어 둔다.
**순서를 바꾸지 않는다.**

| README 수행 순서 | 이 노트 |
| --- | --- |
| 1. 각 Step 뒤 Unstaged 전체 diff로 계획과 대조 | 4장(범위 선택) · 6장(실제 대조) |
| 2. 예상 밖이면 **다음 Step 전에 멈춤** | 6장 화면 1의 「계획에 없는 파일이 있으면」 → E4로 넘김 |
| 3. self-review로 **읽기 전용** 검토 | 7장(요청) · 8장(결과 읽기) |
| 4. 결과를 `notes/diff-review-l12.md`에 기록 | **9장** |

⚠️ **1번은 「각 Step 뒤」다. 수업에서는 한 번만 시연한다 — 4장에서 그 사실을 말하고 간다.**
⚠️ **3번과 4번은 붙이지 않는다.** 읽는 동안에는 쓰지 않는다. **기록은 9장에서 한 번에** 한다.
...
⚠️ **먼저 이걸 말하고 시작한다. 안 하면 학생이 오해한 채로 따라 한다.**
**README 수행 순서 1번은 「각 Step 뒤」다. 그런데 수업에서는 Step마다 다 돌 시간이 없다.**
**한 번만 시연한다는 걸 숨기지 말고 말한다.**
> "원래는 **Step 하나 끝날 때마다** 이걸 합니다. Step이 다섯 개면 **다섯 번**이에요.
> 수업에서는 **시간상 한 번만** 보여 드릴게요. 대신 **여러분이 하실 때는 Step마다** 하세요."
> "**왜 매번 하냐면** — **끝까지 다 하고 나서 보면, 어디서부터 어긋났는지 못 찾습니다.**
> **한 Step씩 확인해야 되돌릴 자리가 좁아져요.**"
