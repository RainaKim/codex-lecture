...
⚠️ **여유가 있으면 한 겹 더 본다. 볼 곳은 딱 한 군데다 —**
**`Goal`에서 「재고가 부족하면 거절한다」에 해당하는 문장 하나.** 그 문장을 짚어 놓고
**아래 세 가지를 물어본다.** 이 세 가지는 **주문 한 건에 상품이 여러 줄 들어올 수 있어서** 생긴다
(`src/orders.ts:59~64`가 그 줄들을 하나씩 도는 루프다).

| 물을 것 | 왜 이게 빠지나 | 안 적혀 있으면 할 말 |
| --- | --- | --- |
| **한 줄만 부족하면 전부 막나?** A 2개 + B 5개인데 **B만 모자란** 경우 | "거절한다"만 쓰면 **A는 어떻게 되는지** 안 정해진 것. 지금 코드는 **A를 이미 깎고** 지나간다 | "**주문 하나에 상품이 여러 개**일 수 있잖아요. **하나만 모자라면 전체를 막나요, 그 줄만 빼나요?** 이게 없네요." |
| **딱 맞으면 되나?** 재고 3개에 **3개** 주문 | "부족하면 거절"과 "넘으면 거절"은 **경계에서 답이 갈린다** | "**재고가 딱 3개인데 3개 주문**하면요? 되는 거죠. 그런데 **이 문장으로는 안 갈립니다.**" |
| **같은 상품이 두 줄이면 합치나?** A 2개, A 2개 | 루프가 **줄마다 따로** 보므로, 재고 3개면 **합치면 거절, 따로 보면 둘 다 통과** | "**같은 상품을 두 줄로** 넣으면요? **합쳐서 4개로 볼지, 각각 2개로 볼지**에 따라 결과가 달라집니다." |

⚠️ **셋 다 물을 필요는 없다. 하나만 짚고 이렇게 닫는다.**
> "이런 건 **구현하다 만나면 늦습니다.** 그때 가서 정하면 **Codex가 알아서 정해 버려요.**
> 지금 계획에 적어 두죠."

⚠️ **세 개가 다 적혀 있으면 그것도 말해 준다. 좋은 신호다.**
> "**성공했을 때만이 아니라 애매한 경우까지** 적혀 있네요. **여러 줄 중 하나만 모자랄 때**,
> **딱 맞아떨어질 때** — 이런 게 **구현을 시작하기 전에 정해져 있으면** 나중에 다툴 일이 없습니다."
...
| 보는 방향 | 물을 것 |
| --- | --- |
| 바꿀 목록 → 근거 | 이 파일은 **어느 완료 기준** 때문에 바꾸나? |
| 읽을 목록 → 역할 | 읽기만 하는 파일인가, **바꿔야 하는데 빠진** 건가? |
...
⚠️ **치면서 세 토막을 소리 내어 말한다. 이게 README의 그 두 줄이다.**

| 친 부분 | 할 말 |
| --- | --- |
| `…l12-inventory-after-payment.md 에 저장해` | "**어디에 저장할지**를 우리가 정해 줍니다." |
| `나머지 다섯 영역은 template 안내 그대로` | "**안 시키면 다 채워 줍니다.** 근데 그건 **우리가 검토도 승인도 안 한 내용**이에요." |
| `src/ tests/ 는 건드리지 마` | "**계획을 저장하는 요청에 코드 수정이 딸려 오면 안 됩니다.**" |
