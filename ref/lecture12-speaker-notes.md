...
### 강사용 수행 순서 — README에서 덜어낸 운영 런북

이 순서는 수강생에게 외울 명령 목록으로 주지 않고, **강사가 E3 라이브 데모를 빠뜨리지 않기 위해**
사용한다. README의 네 단계인 **구현 → 범위 대조 → self-review → 기록**을 실제 화면 조작으로 풀어
쓴 것이다.

1. 승인된 plan의 `Approval Gate`와 `Files to Change`를 먼저 화면에 띄운다.
   **미결 정책이나 미체크 승인이 남아 있으면 구현 요청을 보내지 않는다.**
2. README의 E3 구현 요청을 실행한다. Codex가 한 번에 구현하더라도 **결과를 곧바로 완료로 받아들이지
   않는다.**
3. review pane을 **Unstaged 전체 작업 diff**로 맞춘다. **Last turn만 보지 않고** 이전 단계에서 생긴
   변경과 미추적 파일까지 범위에 넣는다.
4. 아래 명령으로 파일 수·변경 규모·미추적 파일을 확인한 뒤, 전체 diff를 연다.
...
5. **첫 번째 명령**의 파일 목록을 plan의 `Files to Change`와 대조한다. 줄 수는 **자동 합격·실패
   기준이 아니라 예상 밖 확대를 찾는 신호**로만 쓴다.
6. **두 번째 명령**으로 `git diff`에 잡히지 않는 **미추적 파일**을 확인한다. 새 앱·테스트 파일이
   있으면 review pane에서 **내용까지** 읽는다.
7. **세 번째 명령**과 review pane에서 파일 내부의 **분기·호출·저장 순서**를 `Steps`와 대조한다.
   **파일 이름만 일치한다고 범위가 맞다고 판정하지 않는다.**
8. **네 번째 명령**으로 **관리자 표시라는 제외 범위**가 건드려지지 않았는지 확인한다. 라우터까지
   제외 근거를 보여 줄 필요가 있으면 `git diff -- src/admin.ts src/server.ts`로 넓혀 확인한다.
9. 혹시 **stage된 변경**이 보이면 `git diff --cached --stat -- src tests`와 staged diff를 추가
   확인한다. E3에서는 stage나 commit하지 않으므로, staged 변경을 발견했다고
   **Stage all / Revert all로 일괄 처리하지 않는다.**
10. 계획에 연결된 테스트와 검증을 **실행한 실제 결과**를 확보한다. **명령 목록이 계획에 있다는 사실과
    명령을 실행해 통과했다는 사실을 구분한다.**
11. **사람이 먼저** 핵심 diff를 짧게 검토한다. **변경 파일 → 영향이 큰 처리 순서 → 연결된 테스트 →
    제외 범위** 순으로 **대표 항목만** 본다.
12. 그다음 Issue·승인 plan·**미추적 파일을 포함한 전체 diff**·실제 검증 결과를 입력으로 **읽기 전용**
    self-review를 실행한다.
13. finding이 나오면 **원문과 대조해 수용 여부를 사람이** 정한다. 수용한 수정만 **좁게** 반영하고
    관련 검증을 다시 실행한다. 새 정책이나 범위 확대가 필요하면 **E4 판단으로 넘기고 멈춘다.**
14. 마지막으로 결과를 `notes/diff-review-l12.md`의 **다섯 영역**에 기록한다.
    **E3가 끝나도 아직 stage·commit하지 않는다.**
...
| 1~2 | 3장 구현 데모 |
| 3~4 | 4장 review pane |
| 5~9 | 6장 diff를 사람이 읽는다 (화면 1~5) |
| 10 | 5장 검증 수단과 한계 |
| 11 | 6장 앞머리 — **사람이 먼저** |
| 12 | 7장 self-review 데모 |
| 13 | 8장 결과 읽기 |
| 14 | 9장 기록 |

### 이 순서를 왜 이렇게 쓰는지 — 학생에게 할 말

⚠️ **명령을 읽어 주지 않는다. 왜 여러 화면을 보는지만 말한다. 아래 세 덩어리를 쓴다.**

> "README에서는 **해야 할 판단만** 남겼고, 지금 명령들은 제가 **그 판단에 필요한 화면을 빠짐없이
> 확보하기 위한 진행 순서**입니다. **명령 자체가 수업 목표는 아닙니다.** **파일 목록, 실제 코드,
> 테스트 증거를 서로 다른 화면에서 확인해 한쪽의 빈틈을 다른 쪽으로 보완**하는 것이 목표입니다."
...
> "**각 Step마다 모든 명령을 기계적으로 반복해 방송할 필요는 없습니다.** Codex가 **단계별로 멈췄다면
> Step 경계마다** 확인하고, **한 번에 구현했다면 구현 직후 전체 범위**를 확인합니다. **어느 경우든
> self-review보다 사람의 첫 diff 확인이 먼저**입니다."
...
⚠️ **보내기 전에 관문이 하나 있다.** (런북 1)
**승인된 plan의 `Approval Gate`와 `Files to Change`를 먼저 띄운다.**
**미결 정책이나 미체크 승인이 남아 있으면 구현 요청을 보내지 않는다.** 여기서 멈추는 것도 정상이다.

`[화면: plan의 `### Approval Gate` → 위로 스크롤해 `Files to Change`]`
> "보내기 전에 **두 군데만** 확인하겠습니다. **여기 승인 근거가 채워져 있고**, **체크가 되어
> 있는지**. 그리고 **우리가 승인한 파일 목록이 이것**인지."
> "**하나라도 비어 있으면 여기서 멈춥니다.** **승인 안 된 계획으로 구현을 시작하면, 뒤에 하는 대조가
> 전부 의미가 없어져요.**"

⚠️ **오늘 처음으로 `src`·`tests`가 바뀐다. 보내기 전에 한마디 하고 간다.**
> "지금까지는 **문서만** 만들었죠. **여기서부터 코드가 바뀝니다.** 열두 강의 만에 처음이에요."

`[Composer → README E3 `실행`의 요청 그대로]`

⚠️ **요청을 보내기 전에 네 문단이 각각 무슨 일을 하는지 짚는다. 전부 읽지 않는다.**
> `[1문단]` "**승인 기록을 먼저 확인하고, 승인된 범위만** 구현하라고 합니다. **아까 채운 Approval
> Gate를 읽고 시작**하는 거예요."
...
⚠️ **확인 횟수는 Codex가 어떻게 멈췄느냐에 따라 달라진다. 미리 정해 두지 않는다.**
**단계별로 멈췄으면 Step 경계마다, 한 번에 구현했으면 구현 직후 전체 범위를 본다.**
(위 런북의 세 번째 멘트를 여기서 쓴다.)

⚠️ **범위 선택을 먼저 정한다. 이게 4장의 첫 조작이다.** (런북 3)
> "화면에서는 **마지막 응답의 변경만 보지 않고 전체 작업 diff**를 보겠습니다. **Last turn만 보면 이전
> 단계에서 바꾼 내용이 빠질 수** 있어요. **Unstaged 전체 범위**를 확인합니다."

⚠️ **그다음 터미널에서 네 개를 돌린다.** (런북 4) **여기서 화면 네 장을 한 번에 확보한다.**
...
⚠️ **명령을 읽어 주지 않는다. 왜 네 개인지만 말한다.** (런북 두 번째 멘트)
> "**`git diff`만 보면 새 파일을 놓칠 수** 있어서 **status를 같이** 봅니다. 반대로 **stat만 보면
> 파일 안에서 무엇이 바뀌었는지 알 수 없어서 전체 diff를** 봅니다. **제외 파일의 diff가 비어 있는지도
> 확인**해 **'안 건드렸다'는 말을 관찰 가능한 근거로** 바꾸겠습니다."

⚠️ **네 번째 명령이 비어 있는 게 정상이다. 비어 있다는 걸 소리 내어 말한다.**
> "마지막 건 **아무것도 안 나왔죠.** 이게 **'관리자 쪽은 안 건드렸다'는 증거**입니다.
> **말이 아니라 빈 출력이 근거**예요."
> `[라우터까지 보여 줄 필요가 있으면]` **`git diff -- src/admin.ts src/server.ts`로 넓힌다.**

⚠️ **stage된 변경이 보이면 여기서 한 번 더 확인한다.** (런북 9)
`git diff --cached --stat -- src tests`와 staged diff를 본다.
**E3에서는 stage도 commit도 하지 않는다. 발견했다고 Stage all / Revert all로 일괄 처리하지 않는다.**
...
`[Composer — README E3의 self-review 요청 그대로]`
