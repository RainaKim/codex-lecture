...
**3) `[src/orders.ts의 생성 시 차감 줄 → src/payments.ts의 결제 처리부]`를 차례로 짚는다**
> "현재 코드는 **주문을 만들 때** 줄이고 있죠. **목표와 다른 지점이 여기**입니다."

⚠️ **파일 목록이 어디서 나오는지 못 박는다.**
> "목표와 지금 동작이 **어떻게 다른지 알아야 바꿀 책임을 정할 수** 있습니다. **파일 목록은 그 조사
> 결과여야지, 파일 이름이 눈에 띄었다고 정하는 목록이 아닙니다.**"

**4) `[다시 Issue로 돌아오며]`**
> "그래서 **이 두 파일이 이 완료 기준을 책임집니다.** 그리고 끝났다는 건 **재고 수량**으로 확인하고요."

⚠️ **마지막은 관찰 결과로 닫는다. E2로 넘기는 다리다.**
> "**'개선한다'로 끝내지 않습니다.** **응답이나 저장 상태, 재고 수량, 이력** 중 **무엇이 달라지는지**를
> 적어요. 이 결과를 **E2에서 검증 방법과 연결**하겠습니다."

⚠️ **마지막에 거꾸로 한 번 물어본다. 이게 오늘 E1의 핵심 개념이다.**
> "반대로 물어볼게요. **`payments.ts`는 왜 바꾸죠?** …네, **이 완료 기준 때문**입니다."
