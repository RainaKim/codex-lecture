...
`[터미널 — 다섯 줄을 한 번에 붙여 넣고, 출력을 위에서부터 짚는다]`
```bash
git branch --show-current
git rev-parse --short HEAD
git status --short --untracked-files=all
...
⚠️ **출력을 한 줄씩 짚으며 무엇을 보는지 말한다.** 명령을 다섯 개 읽는 게 아니라 **세 가지를 확인**한다.

**첫째·둘째 줄 — `branch`와 `HEAD`**
> "지금 **어느 브랜치의 어느 커밋에서 출발하는지**입니다. **인계 문서에 적힌 값과 같은지** 맞춰
> 보세요. 다르면 **여기서 멈추고** 어느 쪽이 맞는지부터 정해야 합니다."
...
- 브랜치·HEAD가 handoff와 다르면 → "인계받은 상태와 **출발점이 다릅니다.** 여기서 시작하면 나중에
  diff 대조가 전부 어긋나요. **어느 쪽이 맞는지 먼저 정하겠습니다.**"
