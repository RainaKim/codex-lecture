...
`[`Steps`의 제목 줄만 위에서 아래로 훑는다. 본문은 아직 읽지 않는다]`
...
`[Step 하나를 고른다. 그 Step의 `종료 상태`와 `Tests`의 해당 항목을 번갈아 짚는다]`

⚠️ **여기가 2장에서 예고한 자리다.** 전부 보지 말고 **Step 하나만** 고른다.
**입력 근거 → 허용된 변경 → 종료 상태 → 확인 방법**을 한 줄로 따라가면 된다.
...
`[`Risks`와 `Out of Scope` 두 영역을 차례로 본다]`

⚠️ **찾을 것.** **`Risks`는 「무슨 일이 생기나」 + 「그걸 어떻게 알아채나」**가 한 쌍으로 있는가.
**`Out of Scope`는 남이 읽고 판정할 수 있을 만큼 구체적인가.**
위험을 **나열만** 하고 끝나거나, 제외가 **한 단어**로만 적혀 있으면 아쉬운 쪽이다.
...
`[`Stop Conditions`와 `Approval Gate`를 본다]`

⚠️ **찾을 것.** 3장에서 **아직 안 정했다**고 한 것이 **계획에서도 안 정해진 채**로 있는가.
그리고 **멈출 기준이 「성공」 하나뿐인가, 아니면 반복 한도·정체·사람 판단까지 있는가.**
...
> "그리고 **`Approval Gate`는 아직 빈칸**입니다. 이게 맞아요. **체크박스가 있다고 승인된 게
> 아닙니다.** **이건 다음 장에서 제가 직접 채웁니다.**"
